(function ($) {
    ...
    dsnEditor();
    loadMore();
    sidebarCommentsOptions();

    var dsn_full_con = $('.dsn-full .dsn-container')
    var align_left = dsn_full_con.find('> .wp-caption.alignleft, > img.alignleft');
    var align_right = dsn_full_con.find('> .wp-caption.alignright, > img.alignright');
    if (align_left.length > 0 || align_right.length > 0) {
    ...
    function lidaSetProperty() {
        var win_width = wind.width();

        if (win_width > 1240) {
            var num = ((win_width - 1240) / 2) + 15;
            if (align_left.length > 0)
                align_left.css({marginLeft: num});
            if (align_right.length > 0)
                align_right.css({marginRight: num});
    ...
    function sidebarCommentsOptions() {
        ...
        $('.comment-avatar-meta , [data-dsn-close=".dsn-comment"]').on('click', function () {
            if ($(this).hasClass('comment-avatar-meta')) {
                body.addClass('dsn-show-comment');
            } else {
                body.removeClass('dsn-show-comment');
            }
        });
    }
    ...
    function dsnEditor() {
        ...
        $('.post-full-content.single-post > p:empty').remove();
    ...
    function parallax(wind) {
        ...
        var dsn_parallax_move_img = $('.dsn-parallax-move-img');
        dsnGrid.parallaxMoveElemnt(dsn_parallax_move_img, 20, 1);
    ...
    function dsnHeroSection(wind) {
        ...
        var content_background = $('.content-background  , .page-content.parallax-head.dsn-scale-parallax-head.dsn-page-full:not(.dsn__works)');
        var page_content = $('.page-content');
            ...
            if ((num_background + 0.91667) > 1) {
                ...
                if ($(window).width() > 991) {
                    content_background.css({
                        transform: 'translate3d(0px,0px,0px) scale(' + (num_background + 0.92) + ')'
                    });
                ...
                if (header.hasClass('header-single')) {
                    TweenMax.to(header_content, .3, {
                        scale: (1 - num_content_scal),
                        opacity: opacityContent - .3,
                        y: (windowScrolled / -3)
                    });
                } else {
                    TweenMax.to(header_content, .3, {
        ...
        next_up.on('mouseenter', function () {
    ...
    function aosInit() {
        var is_active = lidaParam.effect_aos;
        try {
    ...
    function navBar(wind) {
        ...
        $('.site-header .accent-menu .extend-container > .custom-drop-down > a').on('click', function () {
            return false;
        });
        ...
        $('.nav__list-item:not(.nav__list-dropdown) , .header-top .logo a ').on("click", function () {
            body.removeClass('nav-active');
        });

        $(".nav__list > .nav__list-dropdown > a").on('click',
            function (e) {
                e.preventDefault();
    ...
    function mouseCirMove($off) {
        ...
        function cursorEffect() {
            dsnGrid.elementHover($elemnet, 'a , .to-prev , .to-next , .fas.fa-angle-right , .fas.fa-angle-left , .hero__down , .link-click , .filter-btn , .icon__fixed , .t-link , .button-next , .toggle , input , .dsn-button-sidebar , .close-wind', 'custom-cursor-link');
    ...
    function ajaxLoad($refresh) {
        ...
        var link_menu = $('.site-header a ,.header-top a , .next-proj-slider a , .next-up a , .portfolio .gallery > .projs-item a , a.dsn-ajax-theme , .dsn-pagination a');
        var elemnt_ajax = '';
        ...
        projects__headline.bind('mousedown touchstart', function (e) {
            ...
            tl = new TimelineLite();
            var timer_hold = dsnGrid.getUndefinedVal(lidaParam.hold_time, 1.5);
            tl.fromTo(circlehold, timer_hold, {
        ...
        function animate_ajax_start(url) {
            ...
            var $scroll_top = wind.scrollTop() + (wind.height());
            var doc_hei = $(document).height() - (wind.height() / 2);
            var effect_root = main_root;

            if ($scroll_top >= doc_hei) {
                effect_root = main_root.children;
            }


            TweenMax.fromTo(effect_root, 1, {
                y: 0
            }, {
                y: -150,
                ease: Expo.easeIn
    ...
    function isNumSlid(root_slid) {
        var get_section_slider = $(root_slid).find('.section-slider');

        if (get_section_slider.length > 1) {
            return true;
        }
    ...
    function nextSlid(root_slid, tl) {
        if (!isNumSlid(root_slid)) return false;
        $(root_slid).find('.section-slider')
    ...
    function scrollEvent(locked) {
        ...
        function scroller(wind) {
            ...
            var
                scroll_time_mouse_wheel = dsnGrid.getUndefinedVal(lidaParam.scroll_time_mouse_wheel, .85),
                scroll_distance_mouse_wheel = dsnGrid.getUndefinedVal(lidaParam.scroll_distance_mouse_wheel, 250),
                scroll_time_up_down_arrow = dsnGrid.getUndefinedVal(lidaParam.scroll_time_mouse_wheel, .1),
                scroll_distance_up_down_arrow = dsnGrid.getUndefinedVal(lidaParam.scroll_distance_up_down_arrow, 10),
                scroll_time_up_down_page = dsnGrid.getUndefinedVal(lidaParam.scroll_time_up_down_page, .5),
                scroll_distance_up_down_page = dsnGrid.getUndefinedVal(lidaParam.scroll_distance_up_down_page, 300);
            ...
            wind.on('keydown', function (e) {
                ...
                if (keyCode === 38) {
                    setPositionScroll(1, scroll_time_up_down_arrow, scroll_distance_up_down_arrow);
                } else if (keyCode === 40) {
                    setPositionScroll(-1, scroll_time_up_down_arrow, scroll_distance_up_down_arrow);
                } else if (keyCode === 34) {
                    setPositionScroll(-1, scroll_time_up_down_page, scroll_distance_up_down_page);
                } else if (keyCode === 33) {
                    setPositionScroll(1, scroll_time_up_down_page, scroll_distance_up_down_page);
                } else if (keyCode === 36) {
            ...
            wind.on("mousewheel DOMMouseScroll", function (event) {
                ...
                setPositionScroll(delta, scroll_time_mouse_wheel, scroll_distance_mouse_wheel);

            });

            function setPositionScroll(delta, scrollTime, scrollDistance) {

                var active_element = wind;

                if (body.hasClass('dsn-show-sidebar')) {
                    active_element = $('.dsn-sidebar .sidebar-single');
                } else if (body.hasClass('dsn-show-comment')) {
                    active_element = $('.comment-modal-dark .comment-modal-container');
                } else if (body.hasClass('nav-active')) {
                    active_element = $('.nav__content');
                }


                var scrollTop = active_element.scrollTop();
                var finalScroll = scrollTop - parseInt(delta * scrollDistance);


                TweenLite.to(active_element, scrollTime, {
    ...
    function services_tab() {
        ...
        services.each(function () {
            var _taht = $(this);
            var services_wp = _taht.find('.services-wp');
            var box_title_services = services_wp.find('.box-title-services ul');
            var content = services_wp.find('.services-item-info');
            content.each(function ($index) {
                var _that = $(this);
                var title = _that.data('title');
                _that.attr('id', 'tabs-' + $index + "-content");
                var li = $('<li class="link-click"></li>').attr('id', 'tabs-' + $index).text(title);
                if ($index === 0) {
                    li.addClass('active');
                }
                box_title_services.append(li);
            });
            ...
            _taht.on("click", ".link-click", function () {
                ...
                _taht.find("#" + myID + "-content").fadeIn().siblings().hide();
    ...
    function initMap() {
        ...
        if (map_scropt_id === null) {
            var GOOGLE_MAP_KEY = dsnGrid.getUndefinedVal(lidaParam.map_api, '');
        ...
        setTimeout(function () {
            try {
                ...
                var map = new google.maps.Map(map_id, {
                    center: letLeng,
                    zoom: parseInt(dsnGrid.getUndefinedVal(lidaParam.map_zoom_level, 12)),
                    styles: [
                        {
                            "featureType": "all",
                ...
                var marker = new google.maps.Marker({
                    ...
                    icon: dsnGrid.getUndefinedVal(lidaParam.map_marker_icon, 'assets/img/map-marker.html'),
                    title: 'ASL',
                    map: map
